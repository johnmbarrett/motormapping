function makeTrialVideo(self,locationIndex,bodyPartIndex,trialIndex,varargin) % TODO : make it so that you can exclude bodyPartIndex if it's unnecessary
    ...
    videoFile = sprintf('VT%d',locationIndex-1); % TODO : store this information when generating the motor mapping result
    
    load([videoFile '.mat'],'VT'); % TODO : push file/video handling into its own dedicated wrapper class with support for multiple file formats
    ...
    if parser.Results.TraceTrajectory
        ...
        imageFun = @(I) (128+I/2)/255; % TODO : specify transparency?
        cax = [0 1];
    elseif isPlainVideo
        imageFun = @(I) I;
        cax = [0 255];
    ...
    for ii = 1:size(V,3)
        tic; % TODO : logging
        if isPlainVideo
            writeVideo(w,imageFun(V(yidx,xidx,ii)/255)); % TODO : see above RE video/file handling, also do we *really* need imageFun here?
            toc;
            continue
        end
